...
public class ResourceSupplier {
  ...
  private static final liquibase.resource.ResourceAccessor RESOURCE_ACCESSOR =
      new SimpleResourceAccessor();

  private static final String usersCsv =
      "username, fullname, pk_id\n" + "nvoxland, Nathan Voxland, 1\n" + "bob, Bob Bobson, 2";
    ...
    public Set<InputStream> getResourcesAsStream(String path) throws IOException {
      ...
      if (path.toLowerCase().endsWith("csv")) {
        ...
        stream = new ByteArrayInputStream(
            ((String) ChangeFactory.getInstance().getChangeMetaData(new CreateProcedureChange())
                .getParameters().get("procedureBody").getExampleValue(new HsqlDatabase()))
                    .getBytes());
