...
public abstract class AbstractPreparedStatement implements
  ...
  protected void setParameter(PreparedStatement stmt, List<ColumnConfig> cols,
      String parentFilePath, ResourceAccessor opener) throws SQLException,
      DatabaseException {
  ...
  protected void setParameter(PreparedStatement stmt, int paramStartAt,
      List<ColumnConfig> cols, String parentFilePath, ResourceAccessor opener)
    ...
    for (int i = 0; i < cols.size(); i++) {
      ...
      if (col.getValue() != null) {
        ...
        stmt.setDate(paramIndex,
            new java.sql.Date(col.getValueDate().getTime()));
      } else if (col.getValueBlobFile() != null) {
        try {
          ...
          String file = col.getValueBlobFile();
          InputStream stream = getLobFileStream(opener, file, parentFilePath);
          try{
        	  stmt.setBinaryStream(paramIndex, new BufferedInputStream(stream));
          }catch(Exception ee){
        	  stmt.setBinaryStream(paramIndex, new BufferedInputStream(stream),stream.available());
          }
        } catch (IOException e) {
        ...
        try {
          ...
          String file = col.getValueClobFile();
          InputStream stream = getLobFileStream(opener, file, parentFilePath);
          InputStreamReader streamReader = new InputStreamReader(stream, StreamUtil.getDefaultEncoding());
          try{
        	  stmt.setCharacterStream(paramIndex, new BufferedReader(streamReader));
          }catch(Exception ee){
        	  stmt.setCharacterStream(paramIndex, new BufferedReader(streamReader),stream.available());
          }
        } catch (IOException e) {
          throw new DatabaseException(e.getMessage(), e); // wrap
        }
