...
/**
 * Primary facade class for interacting with Liquibase. The built in command line, Ant, Maven and
 * other ways of running Liquibase are wrappers around methods in this class.
 */
public class Liquibase {
  ...
  /**
   * Creates a Liquibase instance for a given DatabaseConnection. The Database instance used will be
   * found with
  ...
  /**
   * Creates a Liquibase instance. The changeLogFile parameter must be a path that can be resolved
   * by the passed ResourceAccessor. If windows style path separators are used for the
   * changeLogFile, they will be standardized to unix style for better cross-system compatib.
   *
   * @See DatabaseConnection
   * @See Database
   * @see ResourceAccessor
  ...
  public Liquibase(String changeLogFile, ResourceAccessor resourceAccessor, Database database)
    ...
    if (changeLogFile != null) {
      this.changeLogFile = changeLogFile.replace('\\', '/'); // convert to standard / if using
  ...
  public Liquibase(DatabaseChangeLog changeLog, ResourceAccessor resourceAccessor,
      Database database) {
    log = LogFactory.getLogger();
    ...
    if (this.changeLogFile != null) {
      changeLogFile = changeLogFile.replace('\\', '/'); // convert to standard / if using absolute
                                                        // path on windows
    }
    this.resourceAccessor = resourceAccessor;
    this.database = database;
  ...
  /**
   * Use this function to override the current date/time function used to insert dates into the
   * database. Especially useful when using an unsupported database.
   *
  ...
  /**
   * Executes Liquibase "update" logic which ensures that the configured {@link Database} is up to
   * date according to the configured changelog file. To run in "no context mode", pass a null or
  ...
  public void run(DatabaseChangeLog changeLog, Contexts contexts, LabelExpression labelExpression,
      ChangeSetVisitor visitor, ChangeSetFilter[] changeSetFilters)
      throws LiquibaseException, DatabaseException {
    ...
    ChangeLogIterator changeLogIterator =
        getChangelogIterator(changeLog, contexts, changeSetFilters);
    changeLogIterator.run(visitor, new RuntimeEnvironment(database, contexts, labelExpression));
  }

  private void validate(DatabaseChangeLog changeLog, Contexts contexts,
      LabelExpression labelExpression, ChangeSetVisitor visitor)
  ...
  public DatabaseChangeLog getDatabaseChangeLog(String changeLogFile)
      throws LiquibaseException, ChangeLogParseException {
    ChangeLogParser parser =
        ChangeLogParserFactory.getInstance().getParser(changeLogFile, resourceAccessor);
    return parser.parse(changeLogFile, changeLogParameters, resourceAccessor);
  }

  public DatabaseChangeLog getIncludedDatabaseChangeLog(DatabaseChangeLog changeLog)
      throws LiquibaseException, ChangeLogParseException {
    ChangeLogParser parser = ChangeLogParserFactory.getInstance()
        .getParser(changeLog.getPhysicalFilePath(), resourceAccessor);
  ...
  protected ChangeLogIterator getStandardChangelogIterator(Contexts contexts,
      LabelExpression labelExpression, DatabaseChangeLog changeLog) throws DatabaseException {
  ...
  private ChangeSetFilter[] getStandardChangeSetFilter(Contexts contexts,
  ...
  public ChangeLogIterator getChangelogIterator(DatabaseChangeLog changeLog, Contexts contexts,
      ChangeSetFilter... changeSetFilters) throws DatabaseException {
    if (changeLog instanceof IncludedDatabaseChangeLog) {
      return new IncludedChangeLogIterator(this, contexts, (IncludedDatabaseChangeLog) changeLog,
          changeSetFilters);
    } else {
  ...
  public void update(Contexts contexts, LabelExpression labelExpression, Writer output)
      throws LiquibaseException {
    changeLogParameters.setContexts(contexts);
    changeLogParameters.setLabels(labelExpression);
  ...
  public void update(int changesToApply, Contexts contexts, LabelExpression labelExpression)
    ...
    try {
      ...
      ChangeLogIterator logIterator = new ChangeLogIterator(changeLog,
          ...
          new ContextChangeSetFilter(contexts), new LabelChangeSetFilter(labelExpression),
          new DbmsChangeSetFilter(database), new CountChangeSetFilter(changesToApply));

      logIterator.run(createUpdateVisitor(),
  ...
  private void outputHeader(String message) throws DatabaseException {
    ...
    if (connection != null) {
      executor
          .comment("Against: " + connection.getConnectionUserName() + "@" + connection.getURL());
    }
  ...
  public void rollback(int changesToRollback, Contexts contexts, Writer output)
      throws LiquibaseException {
  ...
  public void rollback(int changesToRollback, Contexts contexts, LabelExpression labelExpression)
    ...
    try {
      ...
      ChangeLogIterator logIterator = new ChangeLogIterator(database.getRanChangeSetList(),
          changeLog, new AlreadyRanChangeSetFilter(database.getRanChangeSetList()),
          new ContextChangeSetFilter(contexts), new LabelChangeSetFilter(labelExpression),
          new DbmsChangeSetFilter(database), new CountChangeSetFilter(changesToRollback));
  ...
  public void rollback(String tagToRollBackTo, Contexts contexts, Writer output)
      throws LiquibaseException {
  ...
  public void rollback(String tagToRollBackTo, Contexts contexts, LabelExpression labelExpression)
    ...
    try {
      ...
      ChangeLogIterator logIterator = new ChangeLogIterator(ranChangeSetList, changeLog,
          ...
          new LabelChangeSetFilter(labelExpression), new DbmsChangeSetFilter(database));
  ...
  public void changeLogSync(Contexts contexts, LabelExpression labelExpression, Writer output)
    ...
    LoggingExecutor outputTemplate =
        new LoggingExecutor(ExecutorService.getInstance().getExecutor(database), output, database);
    Executor oldTemplate = ExecutorService.getInstance().getExecutor(database);
  ...
  public void changeLogSync(Contexts contexts, LabelExpression labelExpression)
      throws LiquibaseException {
    changeLogParameters.setContexts(contexts);
    changeLogParameters.setLabels(labelExpression);
  ...
  public void markNextChangeSetRan(Contexts contexts, LabelExpression labelExpression,
      Writer output) throws LiquibaseException {
    changeLogParameters.setContexts(contexts);
    changeLogParameters.setLabels(labelExpression);
  ...
  public void markNextChangeSetRan(Contexts contexts, LabelExpression labelExpression)
    ...
    try {
      ...
      ChangeLogIterator logIterator = new ChangeLogIterator(changeLog,
          new NotRanChangeSetFilter(database.getRanChangeSetList()),
          new ContextChangeSetFilter(contexts), new LabelChangeSetFilter(labelExpression),
          new DbmsChangeSetFilter(database), new CountChangeSetFilter(1));
  ...
  public void futureRollbackSQL(Integer count, Contexts contexts, LabelExpression labelExpression,
      Writer output) throws LiquibaseException {
    changeLogParameters.setContexts(contexts);
    changeLogParameters.setLabels(labelExpression);
    ...
    try {
      ...
      if (count == null) {
        ...
        ChangeLogIterator forwardIterator = new ChangeLogIterator(changeLog,
            new NotRanChangeSetFilter(database.getRanChangeSetList()),
            new ContextChangeSetFilter(contexts), new LabelChangeSetFilter(labelExpression),
            new DbmsChangeSetFilter(database), new CountChangeSetFilter(count));
        final ListVisitor listVisitor = new ListVisitor();
        forwardIterator.run(listVisitor,
            new RuntimeEnvironment(database, contexts, labelExpression));

        logIterator = new ChangeLogIterator(changeLog,
            new NotRanChangeSetFilter(database.getRanChangeSetList()),
            new ContextChangeSetFilter(contexts), new LabelChangeSetFilter(labelExpression),
            new DbmsChangeSetFilter(database), new ChangeSetFilter() {
              @Override
  ...
  public void updateTestingRollback(Contexts contexts, LabelExpression labelExpression)
      throws LiquibaseException {
    changeLogParameters.setContexts(contexts);
    changeLogParameters.setLabels(labelExpression);
  ...
  public void checkLiquibaseTables(boolean updateExistingNullChecksums,
      DatabaseChangeLog databaseChangeLog, Contexts contexts, LabelExpression labelExpression)
      throws LiquibaseException {
  ...
  public void upgradeChecksums(DatabaseChangeLog databaseChangeLog, Contexts contexts,
  ...
  /**
   * Returns true if it is "save" to migrate the database. Currently, "safe" is defined as running
   * in an output-sql mode or against a database on localhost. It is fine to run Liquibase against a
   * "non-safe" database, the method is mainly used to determine if the user should be prompted
   * before continuing.
   */
  ...
  public void reportLocks(PrintStream out) throws LiquibaseException {
    ...
    out.println(
        "Database change log locks for " + getDatabase().getConnection().getConnectionUserName()
  ...
  public List<ChangeSet> listUnrunChangeSets(Contexts contexts, LabelExpression labels)
      throws LiquibaseException {
    changeLogParameters.setContexts(contexts);
  ...
  /**
   * Returns the ChangeSetStatuses of all changesets in the change log file and history in the order
   * they would be ran.
   */
  public List<ChangeSetStatus> getChangeSetStatuses(Contexts contexts,
    ...
    ChangeLogIterator logIterator =
        getStandardChangelogIterator(contexts, labelExpression, changeLog);
  ...
  public void reportStatus(boolean verbose, Contexts contexts, Writer out)
      throws LiquibaseException {
  ...
  public Collection<RanChangeSet> listUnexpectedChangeSets(Contexts contexts,
    ...
    ChangeLogIterator logIterator =
        new ChangeLogIterator(changeLog, new ContextChangeSetFilter(contexts),
            new LabelChangeSetFilter(labelExpression), new DbmsChangeSetFilter(database));
  ...
  public void reportUnexpectedChangeSets(boolean verbose, String contexts, Writer out)
      throws LiquibaseException {
    reportUnexpectedChangeSets(verbose, new Contexts(contexts), new LabelExpression(), out);
  }

  public void reportUnexpectedChangeSets(boolean verbose, Contexts contexts,
      LabelExpression labelExpression, Writer out) throws LiquibaseException {
    changeLogParameters.setContexts(contexts);
    changeLogParameters.setLabels(labelExpression);

    try {
      Collection<RanChangeSet> unexpectedChangeSets =
          listUnexpectedChangeSets(contexts, labelExpression);
  ...
  public void clearCheckSums() throws LiquibaseException {
    ...
    try {
      ...
      UpdateStatement updateStatement = new UpdateStatement(getDatabase().getLiquibaseCatalogName(),
          getDatabase().getLiquibaseSchemaName(), getDatabase().getDatabaseChangeLogTableName());
  ...
  public CheckSum calculateCheckSum(final String filename, final String id, final String author)
    ...
    final DatabaseChangeLog changeLog =
        ChangeLogParserFactory.getInstance().getParser(this.changeLogFile, resourceAccessor)
    ...
    if (changeSet == null) {
      throw new LiquibaseException(new IllegalArgumentException(
          "No such changeSet: " + filename + "::" + id + "::" + author));
    }
  ...
  public void generateDocumentation(String outputDirectory, Contexts contexts,
    ...
    try {
      ...
      ChangeLogIterator logIterator =
          new ChangeLogIterator(changeLog, new DbmsChangeSetFilter(database));
  ...
  public DiffResult diff(Database referenceDatabase, Database targetDatabase,
    ...
    return DiffGeneratorFactory.getInstance().compare(referenceDatabase, targetDatabase,
        compareControl);
  }
   ...
   * Safe properties are the ones that doesn't have side effects in liquibase state and also don't
   * change in during the liquibase execution
   * 
  ...
  public void generateChangeLog(CatalogAndSchema catalogAndSchema, DiffToChangeLog changeLogWriter,
      PrintStream outputStream, EbaoDiffOutputControl ebaoDiffOutputControl,
      Class<? extends DatabaseObject>... snapshotTypes)
      throws DatabaseException, IOException, ParserConfigurationException {
    generateChangeLog(catalogAndSchema, changeLogWriter, outputStream, null, ebaoDiffOutputControl,
        snapshotTypes);
  }

  public void generateChangeLog(CatalogAndSchema catalogAndSchema, DiffToChangeLog changeLogWriter,
      PrintStream outputStream, EbaoDiffOutputControl ebaoDiffOutputControl, String snapshotTypes)
      throws DatabaseException, IOException, ParserConfigurationException {
    generateChangeLog(catalogAndSchema, changeLogWriter, outputStream, null, ebaoDiffOutputControl,
        snapshotTypes);
  }
  ...
  public void generateChangeLog(CatalogAndSchema catalogAndSchema, DiffToChangeLog changeLogWriter,
      ...
      EbaoDiffOutputControl ebaoDiffOutputControl, String snapshotTypes)
      throws DatabaseException, IOException, ParserConfigurationException {
    ...
    SnapshotControl snapshotControl =
        new EbaoSnapshotControl(this.getDatabase(), types, ebaoDiffOutputControl);
    CompareControl compareControl = new EbaoCompareControl(
        new CompareControl.SchemaComparison[] {
            new CompareControl.SchemaComparison(catalogAndSchema, catalogAndSchema)},
        snapshotTypes, ebaoDiffOutputControl);
    ...
    try {
      originalDatabaseSnapshot = SnapshotGeneratorFactory.getInstance().createSnapshot(
          compareControl.getSchemas(CompareControl.DatabaseRole.REFERENCE), getDatabase(),
          snapshotControl);
      DiffResult diffResult = DiffGeneratorFactory.getInstance().compare(originalDatabaseSnapshot,
          SnapshotGeneratorFactory.getInstance().createSnapshot(
              compareControl.getSchemas(CompareControl.DatabaseRole.REFERENCE), null,
              snapshotControl),
          compareControl);
  ...
  public void generateChangeLog(CatalogAndSchema catalogAndSchema, DiffToChangeLog changeLogWriter,
      PrintStream outputStream, ChangeLogSerializer changeLogSerializer,
      EbaoDiffOutputControl ebaoDiffOutputControl, Class<? extends DatabaseObject>... snapshotTypes)
      throws DatabaseException, IOException, ParserConfigurationException {
    Set<Class<? extends DatabaseObject>> finalCompareTypes = null;
    if (snapshotTypes != null && snapshotTypes.length > 0) {
      finalCompareTypes =
          new HashSet<Class<? extends DatabaseObject>>(Arrays.asList(snapshotTypes));
    }

    SnapshotControl snapshotControl =
        new EbaoSnapshotControl(this.getDatabase(), ebaoDiffOutputControl, snapshotTypes);
    CompareControl compareControl = new EbaoCompareControl(
        new CompareControl.SchemaComparison[] {
            new CompareControl.SchemaComparison(catalogAndSchema, catalogAndSchema)},
        finalCompareTypes, ebaoDiffOutputControl);
    // compareControl.addStatusListener(new OutDiffStatusListener());
  ...
  public void generateChangeLog(CatalogAndSchema catalogAndSchema, DiffToChangeLog changeLogWriter,
    ...
    EbaoDiffOutputControl ebaoDiffOutputControl =
        new EbaoDiffOutputControl(false, false, true, database);
    SnapshotControl snapshotControl =
        new EbaoSnapshotControl(this.getDatabase(), ebaoDiffOutputControl, snapshotTypes);
    CompareControl compareControl = new EbaoCompareControl(
        new CompareControl.SchemaComparison[] {
